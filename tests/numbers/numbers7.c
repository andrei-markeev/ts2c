...
int main(void) {
    ...
    printf("%s\n", js_var_isnan(js_var_from_str("python")) ? "true" : "false");
    ...
    printf("%s\n", js_var_isnan(js_var_from_array(tmp_array)) ? "true" : "false");
    DICT_CREATE(tmp_obj, 4);
    printf("%s\n", js_var_isnan(js_var_from_dict(tmp_obj)) ? "true" : "false");
    printf("%s\n", js_var_isnan(js_var_from_uint8_t(TRUE)) ? "true" : "false");
    printf("%s\n", js_var_isnan(js_var_from(JS_VAR_UNDEFINED)) ? "true" : "false");
    free(tmp_array->data);
    free(tmp_array);
