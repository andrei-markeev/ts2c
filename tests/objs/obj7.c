...
static int16_t i;
static char * tmp_result;
static int16_t j;

int main(void) {
    DICT_CREATE(obj, 4);
    i = 10;
    tmp_result = malloc(STR_INT16_T_BUFLEN);
    assert(tmp_result != NULL);
    sprintf(tmp_result, "%d", i + 1);
    DICT_SET(obj, tmp_result, "test");
    DICT_SET(obj, "12", "yep");
    printf("{ ");
    for (j = 0; j < obj->index->size; j++) {
        if (j != 0)
            printf(", ");
        printf("\"%s\": ", obj->index->data[j]);
        printf("\"%s\"", obj->values->data[j]);
    }
    printf(" }\n");
    free(obj->index->data);
    free(obj->index);
