...
static struct closure_t * inner0;
static struct closure_t * inner1;

int16_t func(struct closure_t * closure)
{
    return closure->scope->x;
}
DICT(struct closure_t *) countTheNumber()
{
    struct scope_t * scope;
    DICT(struct closure_t *) arrToStore;
    ...
    DICT_CREATE(arrToStore, 4);
    ARRAY_PUSH(gc_main_dicts, (void *)arrToStore);
    scope->x = 0;
    for (;scope->x < 9;scope->x++)
    {
        struct closure_t * closure;
        tmp_result = malloc(strlen("func") + STR_INT16_T_BUFLEN + 1);
        assert(tmp_result != NULL);
        tmp_result[0] = '\0';
        strcat(tmp_result, "func");
        str_int16_t_cat(tmp_result, scope->x);
        ...
        closure->func = func;
        closure->scope = scope;
        DICT_SET(arrToStore, tmp_result, closure);
    }
    return arrToStore;
}
...
int main(void) {
    ...
    callInnerFunctions = countTheNumber();
    inner0 = DICT_GET(callInnerFunctions, "func0", 0);
    inner1 = DICT_GET(callInnerFunctions, "func1", 0);
    printf("%d\n", inner0->func(inner0));
    printf("%d\n", inner1->func(inner1));
