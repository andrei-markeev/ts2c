...
int main(void) {
    ...
    tmp_array->data[0] = js_var_from_int16_t(11);
    tmp_array->data[1] = js_var_from_int16_t(22);
    tmp_array->data[2] = js_var_from_int16_t(33);
    arr = js_var_from_array(tmp_array);
    switch (arr.type) {
        case JS_VAR_ARRAY:
            ARRAY_INSERT(((struct array_js_var_t *)arr.data), 0, js_var_from_int16_t(55));
            ARRAY_INSERT(((struct array_js_var_t *)arr.data), 0, js_var_from_int16_t(44));
            arr_size = ((struct array_js_var_t *)arr.data)->size;
            tmp = arr_size;
            break;
        case JS_VAR_NULL:
            ARRAY_PUSH(err_defs, "TypeError: Cannot read properties of null (reading 'unshift')");
            THROW(err_defs->size);
            break;
        case JS_VAR_UNDEFINED:
            ARRAY_PUSH(err_defs, "TypeError: Cannot read properties of undefined (reading 'unshift')");
            THROW(err_defs->size);
            break;
        default:
            ARRAY_PUSH(err_defs, "TypeError: arr.unshift is not a function.");
            THROW(err_defs->size);
            break;
    }
    count = tmp;
    ...
    tmp_obj->count = count;
    printf("{ ");
    js_var_log("arr: ", tmp_obj->arr, "", TRUE, FALSE);    printf(", ");
    printf("count: %d", tmp_obj->count);
    printf(" }\n");
    free(tmp_array->data);
    free(tmp_array);
    free(tmp_obj);
