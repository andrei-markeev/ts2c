...
int main(void) {
    ...
    tuple[0] = js_var_from_int16_t(1);
    tuple[1] = js_var_from_str("hello");
    tuple[2] = js_var_from_uint8_t(TRUE);
    printf("[ ");
    for (i = 0; i < 3; i++) {
        if (i != 0)
            printf(", ");
        js_var_log("", tuple[i], "", TRUE, FALSE);
    }
    printf(" ]\n");
