...
struct array_js_var_t * tmp_array_2 = NULL;
int main(void) {
    ARRAY_CREATE(err_defs, 2, 0);

    x = js_var_from_int16_t(0);
    ARRAY_CREATE(tmp_array, 2, 1);
    tmp_array->data[0] = js_var_from_int16_t(5);
    y = js_var_from_array(tmp_array);
    printf("%s\n", tmp_str = js_var_to_str(x, &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
    printf("%s\n", tmp_str = js_var_to_str(js_var_to_number(y), &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
    x = js_var_get(y, x);
    if (js_var_to_bool(x))
    {
        ARRAY_CREATE(tmp_array_2, 4, 4);
        tmp_array_2->data[0] = js_var_from_int16_t(1);
        tmp_array_2->data[1] = js_var_from_int16_t(2);
        tmp_array_2->data[2] = js_var_from_int16_t(3);
        tmp_array_2->data[3] = x;
        x = js_var_from_array(tmp_array_2);
    }
    printf("%s\n", tmp_str = js_var_to_str(x, &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
    y = js_var_from_str("hello");
    printf("%s\n", tmp_str = js_var_to_str(y, &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
    free(tmp_array->data);
    free(tmp_array);
