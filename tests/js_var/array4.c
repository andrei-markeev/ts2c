...
int main(void) {
    ...
    tmp_array->data[0] = js_var_from_int16_t(88);
    tmp_array->data[1] = js_var_from_int16_t(77);
    tmp_array->data[2] = js_var_from_int16_t(66);
    arr = js_var_from_array(tmp_array);
    switch (arr.type) {
        case JS_VAR_ARRAY:
            value = ((struct array_js_var_t *)arr.data)->data[0];
            ARRAY_REMOVE(((struct array_js_var_t *)arr.data), 0, 1);
            tmp = value;
            break;
        case JS_VAR_NULL:
            ARRAY_PUSH(err_defs, "TypeError: Cannot read properties of null (reading 'shift')");
            THROW(err_defs->size);
            break;
        case JS_VAR_UNDEFINED:
            ARRAY_PUSH(err_defs, "TypeError: Cannot read properties of undefined (reading 'shift')");
            THROW(err_defs->size);
            break;
        default:
            ARRAY_PUSH(err_defs, "TypeError: arr.shift is not a function.");
            THROW(err_defs->size);
            break;
    }
    firstEl = tmp
    ...
    printf("{ ");
    printf(tmp_obj->firstEl.type == JS_VAR_STRING ? "firstEl: \"%s\"" : "firstEl: %s", tmp_str = js_var_to_str(tmp_obj->firstEl, &tmp_need_dispose));
        if (tmp_need_dispose)
            free((void *)tmp_str);
    printf(" }\n");
    free(tmp_array->data);
