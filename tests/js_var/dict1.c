...
int main(void) {
    ...
    DICT_SET(tmp_obj, "tagName", js_var_from_str("h1"));
    dict = js_var_from_dict(tmp_obj);
    if (dict.type == JS_VAR_DICT)
        DICT_SET(((struct dict_js_var_t *)dict.data), "tagName", js_var_from_str("h2"))
    printf("%s\n", tmp_str = js_var_to_str(js_var_get(dict, js_var_from_str("tagName")), &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
    dict = js_var_from_int16_t(1);
    printf("%s\n", tmp_str = js_var_to_str(dict, &tmp_need_dispose));
    if (tmp_need_dispose)
        free((void *)tmp_str);
